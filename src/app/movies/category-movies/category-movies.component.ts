    ...
    if(this.moviesService.catMovies.length === 0 && this.moviesOfCategory.length === 0 && this.currentRoute.snapshot.url.length === 0) {
      ...
      if(this.moviesService.moviesCategories.length === 0) {
        this.moviesService.getCategories().subscribe({
          next: categories => {
            this.categoryName = categories[5].category_name;
            this.moviesService.catName = categories[5].category_name;
            this.categoryId = categories[5].category_id;
        ...
        this.categoryName = this.moviesService.moviesCategories[5].category_name;
        this.categoryId = this.moviesService.moviesCategories[5].category_id;
        this.router.navigate(['/movies', this.categoryName, this.categoryId]);
        this.isLoading = false;
      }
    }else {
